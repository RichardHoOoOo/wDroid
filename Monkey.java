...
public class Monkey {
    ...
    private String packageName;//jiajun: app's package name
    
    private String appName;//jiajun: app's name
    
    public static String currentActivity = ""; //jiajun: name of the current activity
    
    private long allowedRunningTime;
    
    public static boolean canEnterHome = false;
    ...
    private class ActivityController extends IActivityController.Stub {
        public boolean activityStarting(Intent intent, String pkg) {
            ...
            if (mVerbose > 0) {
                ...
                if(allow) {
                    currentActivity = intent.getComponent().getClassName();
                }
        ...
        try {
            ...
            while ((opt = nextOption()) != null) {
                if (opt.equals("-s")) {
                    ...
                    String tmp = nextOptionData();//jiajun: retrieve app name
                    appName = tmp.replace("_", " ");//jiajun:replace _ with space in appName
                } else if(opt.equals("-t")) {
                    allowedRunningTime = Long.parseLong(nextOptionData());//jiajun: retrieve maximum allowed running time
                } 
                else if (opt.equals("-p")) {
                    packageName = nextOptionData();//jiajun: retrieve package name here
                    validPackages.add(packageName);
                    //validPackages.add(nextOptionData()); jiajun: original monkey code commented by me
    ...
    private int runMonkeyCycles() {
        ...
        while (!systemCrashed && cycleCounter < mCount) {
            //If exceed allowed running time, stop
            if(((System.nanoTime() - startTime) / 1000000) >= (allowedRunningTime * 1000)) break;
            synchronized (this) {
                if (mRequestProcRank) {
                    reportProcRank();
...
//                    normalExit = false;
//                    while(attempt < MonkeyView.MAX_ATTEMPS) {
//                        if(MonkeyView.getUILayout(finalScreen) & MonkeyView.isInAppPackage) {
//                            normalExit = true;
//                            break;
//                        } else {
//                            attempt++;
